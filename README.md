...
- **LLM:** Cerebras (`gpt-oss-120b` by default) for classification and chat; Groq optional via `LLM_PROVIDER=groq`
...
| Variable | Purpose |
|----------|---------|
| `LLM_PROVIDER` | `cerebras` (default) or `groq` |
| `CEREBRAS_API_KEY` | Live LLM classification and chat (Cerebras) |
| `CEREBRAS_MODEL` | Model slug (default `gpt-oss-120b`) |
| `LLM_DAILY_TOKEN_BUDGET` | Optional cap for quota-split UI (default `1000000` TPD on Cerebras) |
| `LLM_REQUESTS_PER_DAY` | Daily request budget for split UI (default `2400` on Cerebras) |
...
| `GROQ_API_KEY` | Optional — set `LLM_PROVIDER=groq` to use Groq instead |
| `GROQ_MODEL` | Groq model slug (default `meta-llama/llama-4-scout-17b-16e-instruct`) |
